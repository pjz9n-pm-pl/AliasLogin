...
class AliasLoginNowCommand extends BaseSubCommand
{
    public function __construct()
    {
        parent::__construct(
            "now",
            LanguageHolder::get()->translateString("command.aliaslogin.now.description")
        );
    }
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        ...
        $state = LanguageHolder::get()->translateString("word.none");
        if (($alias = AliasLoginFlag::get($target->getClientId())) !== null) {
            $state = $alias;
        }
        $sender->sendMessage(LanguageHolder::get()->translateString("aliaslogin.now", [$state]));
    }
}
